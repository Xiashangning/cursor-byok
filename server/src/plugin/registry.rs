...
impl PluginRegistry {
    ...
    pub async fn set_model_enabled(
        ...
        let id = super::descriptor::model_id(plugin_id, provider_id, model_id);
        let model = self.model_descriptor(&id).await?;
        self.inner
            .store
            .set_plugin_model_enabled(&model.id, enabled)
            .await
    }
